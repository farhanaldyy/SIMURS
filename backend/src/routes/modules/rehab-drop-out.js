...
const ctrl = require('../../controllers/modules/rehab-drop-out.controller');
...
router.post('/', [
  ...
  body('jumlah_drop_out').isInt({ min: 0 }).withMessage('Jumlah drop out harus berupa angka non-negatif'),
], [
  (req, res, next) => {
...
router.put('/:id', [
  ...
  body('jumlah_drop_out').optional().isInt({ min: 0 }).withMessage('Jumlah drop out harus berupa angka non-negatif'),
], [
  (req, res, next) => {
    if (req.body.tanggal) req.body.tanggal = new Date(req.body.tanggal);
    next();
