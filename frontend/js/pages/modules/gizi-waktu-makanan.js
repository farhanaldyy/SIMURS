...
export default createGenericIndicatorPage({
  title: 'Ketepatan Waktu Pemberian Makanan Kepada Pasien',
  subtitle: 'Pencatatan ketepatan waktu pemberian makanan pasien rawat inap',
  endpoint: '/gizi-waktu-makanan',
  metricType: 'compliance',
  columns: [
    { label: 'Tanggal', key: 'tanggal', render: (r) => new Date(r.tanggal).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) },
    { label: 'Jumlah Tepat Waktu', key: 'jumlah_tepat_waktu' },
    { label: 'Jumlah Porsi', key: 'jumlah_porsi' },
    { label: 'Persentase', key: 'persentase', render: (r) => `${r.persentase}%` }
  ],
  fields: [
    { name: 'tanggal', label: 'Tanggal', type: 'date', required: true, row: 1 },
    { name: 'jumlah_tepat_waktu', label: 'Jumlah Ketepatan Waktu Pemberian Makanan', type: 'number', required: true, row: 2 },
  ...
  calculateSummaryHTML(s) {
    return `
      ...
      <div class="summary-item">
        ...
        <div class="summary-label">Total Ketepatan (Rata-rata: ${s.avg_numerator || 0}/hari)</div>
      </div>
      <div class="summary-item">
      ...
      <div class="summary-item">
        ${renderBadge(s.persen || 0, s.standar || '> 90%')}
        <div class="summary-label" style="margin-top:8px">Standar: ${s.standar || '> 90%'}</div>
      </div>
    `;
  }
});
