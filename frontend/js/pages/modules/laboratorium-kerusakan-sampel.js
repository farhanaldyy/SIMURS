...
const page = createGenericIndicatorPage({
  title: 'Tidak Adanya Kerusakan Sampel',
  subtitle: 'Tidak adanya kerusakan sampel di laboratorium',
  endpoint: '/laboratorium-kerusakan-sampel',
  columns: [
    { 
      label: 'Tanggal', 
    ...
    { label: 'Jumlah Kerusakan Sampel', key: 'jumlah_kerusakan' },
    { label: 'Keterangan', key: 'keterangan' },
    { 
      label: 'Persentase Bebas Kerusakan', 
  ...
  calculateSummaryHTML: (s) => {
    ...
    const totalKerusakan = s.total_kerusakan || 0;
    ...
    return `
      <div class="summary-card-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; width: 100%;">
        ...
        <div class="summary-item"><div class="summary-value">${totalKerusakan}</div><div class="summary-label">Total Kerusakan</div></div>
        <div class="summary-item"><div class="summary-value">${presentase}%</div><div class="summary-label">Persentase Bebas Kerusakan</div></div>
      ...
      <div class="summary-note" style="margin-top: 20px; padding: 16px; background-color: var(--bg-secondary, #f8f9fa); border-left: 4px solid var(--primary-color, #007bff); border-radius: 4px; font-size: 0.95rem; line-height: 1.5; color: var(--text-primary, #333);">
        ...
        Berdasarkan data dapat disimpulkan bahwa <strong>${presentase}%</strong> tidak adanya kerusakan sampel darah pasien di laboratorium.<br>
        Standar pelayanan laboratorium yang ditetapkan oleh menkes 2008: 100%, sehingga dapat disimpulkan bahwa pelayanan laboratorium <strong>${statusText}</strong>.
      </div>
    `;
  }
});
