// Identifikasi Pasien page
import Store from '../../store.js';
import * as api from '../../api/modules/identifikasi-pasien.js';
...
export async function render(container) {
  container.innerHTML = `
    <div class="module-page">
      <div class="page-header"><div><h1 class="page-title">Identifikasi Pasien</h1><p class="page-subtitle">Kepatuhan identifikasi pasien sebelum tindakan</p></div><button class="btn btn-primary" id="btn-add">+ Tambah Data</button></div>
...
function renderDataTable() {
  const columns = [
    ...
    { label: 'Nutrisi', render: (r) => idLabel(r.nutrisi_ngt) },
    { label: 'Darah', render: (r) => idLabel(r.pemberian_darah) },
    { label: 'Tindakan', render: (r) => idLabel(r.tindakan_keperawatan) },
...
function openFormModal(data = null) {
  ...
  const formHTML = `
    <form id="modul-form">
      <div class="form-group"><label class="form-label">Tanggal <span class="required">*</span></label><input type="date" name="tanggal" class="form-control" value="${data?.tanggal?.substring(0, 10) || ''}"></div>
      ...
      <div class="form-row"><div class="form-group"><label class="form-label">Pemberian Obat</label><select name="pemberian_obat" class="form-control">${selOpts(data?.pemberian_obat)}</select></div><div class="form-group"><label class="form-label">Nutrisi NGT</label><select name="nutrisi_ngt" class="form-control">${selOpts(data?.nutrisi_ngt)}</select></div></div>
      <div class="form-row"><div class="form-group"><label class="form-label">Pemberian Darah</label><select name="pemberian_darah" class="form-control">${selOpts(data?.pemberian_darah)}</select></div><div class="form-group"><label class="form-label">Tindakan Keperawatan</label><select name="tindakan_keperawatan" class="form-control">${selOpts(data?.tindakan_keperawatan)}</select></div></div>
    </form>`;
  showModal(isEdit ? 'Edit' : 'Tambah', formHTML, { confirmText: 'Simpan', onConfirm: async () => {
    ...
    const errors = validateForm({ nama_pasien: validateRequired(fd.nama_pasien, 'Nama'), no_rm: validateRequired(fd.no_rm, 'No RM'), tanggal: validateRequired(fd.tanggal, 'Tanggal') });
