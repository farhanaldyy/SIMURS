...
const page = createGenericIndicatorPage({
  ...
  subtitle: 'Kepatuhan Identifikasi Pasien Laboratorium',
  endpoint: '/laboratorium-kepatuhan-identifikasi',
  columns: [
    { 
      label: 'Tanggal', 
    ...
    { label: 'Jumlah Kepatuhan', key: 'jumlah_kepatuhan' },
  ...
  fields: [
    ...
    {
      name: 'jumlah_kepatuhan',
      label: 'Jumlah Kepatuhan',
      type: 'number',
      required: true,
      row: 2
    },
  ...
  calculateSummaryHTML: (s) => {
    ...
    const totalKepatuhan = s.total_kepatuhan || 0;
    ...
    return `
      <div class="summary-card-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; width: 100%;">
        ...
        <div class="summary-item"><div class="summary-value">${totalKepatuhan}</div><div class="summary-label">Total Kepatuhan</div></div>
        <div class="summary-item"><div class="summary-value">${presentase}%</div><div class="summary-label">Persentase Kepatuhan</div></div>
      ...
      <div class="summary-note" style="margin-top: 20px; padding: 16px; background-color: var(--bg-secondary, #f8f9fa); border-left: 4px solid var(--primary-color, #007bff); border-radius: 4px; font-size: 0.95rem; line-height: 1.5; color: var(--text-primary, #333);">
        ...
        Berdasarkan data dapat disimpulkan bahwa kepatuhan identifikasi pasien <strong>${presentase}%</strong> standar pelayanan lab yang ditetapkan oleh Menkes 2008: 100%, sehingga pelayanan lab <strong>${statusText}</strong>.
      </div>
    `;
  }
});
